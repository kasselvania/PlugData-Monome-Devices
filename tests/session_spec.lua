...
test("session prefixes cannot create double-slash capability addresses", function()
    local target = session("one")
    local ok, err = target:set_prefix("/monome/")
    equal(ok, nil)
    equal(err, "invalid_prefix")
    equal(target.prefix, "/monome")
end)
