...
The workbench now contains `monome-discovery`, `monome-registry`, and
`monome-session`. Together they:
...
not claim that Step 1's menu acceptance is complete. The Step 2 session layer
does not depend on that widget and has passed fake-device PlugData acceptance.
Physical hardware acceptance starts with the Grid capability work.
